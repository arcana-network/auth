...
const REDIRECT_WAIT_TIMEOUT = 6000
...
const iconSrc = {
  ...
  fail: (_: Theme) => {
    const strokeColor = 'B43030'
    return `data:image/svg+xml,%3Csvg width='82' height='82' viewBox='0 0 82 82' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M26 55.999L56 26' stroke='%23${strokeColor}' stroke-width='4' stroke-linecap='round'/%3E%3Cpath d='M56 56L26 26' stroke='%23${strokeColor}' stroke-width='4' stroke-linecap='round'/%3E%3Ccircle cx='41' cy='41' r='39' stroke='%23${strokeColor}' stroke-width='4'/%3E%3C/svg%3E%0A`
  },
}
...
const createOverlayOnRedirection = (
  ...
  const timer = setTimeout(() => {
    clearProgress()
    appendSuccessOrFailElements(overlay, theme, 'fail')
  ...
  // on connect show success
  provider.on('connect', () => {
    clearTimeout(timer)
    clearProgress()
    appendSuccessOrFailElements(overlay, theme, 'success')
  })
}

const appendSuccessOrFailElements = (
  overlay: HTMLElement,
  theme: Theme,
  status: 'success' | 'fail'
) => {
  const icon = createDomElement('img', {
    src: iconSrc[status](theme),
  })
  ...
  const text = createDomElement(
    ...
    status === 'success'
      ...
      : 'Please use a different browser or contact the administrator'
  )
